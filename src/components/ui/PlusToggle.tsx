 ...
 */
export function PlusToggle({ open, size = 20, className = "" }: { open: boolean; size?: number; className?: string }) {
  ...
  return (
    <span aria-hidden className={`relative inline-block ${className}`} style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 block" style={{ transform: `rotate(${open ? -45 : 0}deg)`, transition }}>
