...
export function BossInputCard({
  ...
  const availabilityText =
    isGroup && !sameAvail ? formeAvails.map((a) => `${a.label}: ${a.avail}`).join(" · ") : describeAvailability(formes[0]);
  ...
  if (sameSpeciesGroup) {
    ...
    formePretitle = fusedFormes.length >= 2 ? `${fusedFormes.join(", ")}, ${baseForme}` : `${baseForme} & ${fusedFormes[0]}`;
  }
  const titleSprites =
  ...
  const planWindows = isGroup
    ? Array.from(new Map(planningWindows(boss).map((w) => [`${w.day}-${w.startHour}-${w.endHour}`, w])).values())
    : boss.windows;
  ...
  const needEntries = Object.entries(result.needs) as [
    Currency,
    { needed: number; raidsRange: { min: number; max: number } },
  ][];
  ...
  const explainFor = (c: Currency, n: { needed: number; raidsRange: { min: number; max: number } }) => {
    ...
    return ex && ex.needed === n.needed && ex.raids.min === n.raidsRange.min && ex.raids.max === n.raidsRange.max
      ? ex
      : null;
  };
  ...
  return (
    <div
      id={`card-${boss.id}`}
      className="cv-auto enamel tilt-card relative scroll-mt-20 rounded-2xl p-2"
        ...
        <div className="card-text-legible relative z-10 p-3">
          ...
          <div className="relative">
            ...
            <button type="button" onClick={() => setOpen((o) => !o)} aria-expanded={open} className="block w-full text-left">
              ...
              <CardTitle name={displayName} types={cardTypes} isMega={isMega} pretitle={formePretitle} />
                    ...
                    {boosts.length > 0 ? (
                      ...
                      >
                        ...
                        <p className="mt-1.5 text-[12px] text-slate-500">
                          Evolve one (Mega Level 3-4) before battling — its type matches {f.name}, so every raid &amp; wild
                          catch drops bonus Candy XL. Only one mega counts at a time.
                        </p>
                      </Copyable>
                    ) : null}
                  </div>
