...
export function CardJumpNav({ bosses, mewtwoSelected }: { bosses: RaidBoss[]; mewtwoSelected: boolean }) {
  ...
  return (
    <nav
      ...
      // Sticks to the top of the viewport (the step nav now lives at the bottom)
      // and follows the scroll for the whole card stack — in reach from anywhere.
      className="sticky top-0 z-20 flex gap-1 overflow-x-auto rounded-lg border border-white/10 bg-gofest-bg/90 px-2 py-1.5 backdrop-blur-sm [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
    >
      {mewtwoSelected ? (
        <button
          type="button"
