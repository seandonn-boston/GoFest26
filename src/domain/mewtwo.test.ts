...
describe("computeMewtwoResults", () => {
  ...
  it("independent X & Y mega levels per individual (X0/Y3)", () => {
    const copies: PokemonCopy[] = [
      { id: "a", variant: "standard", current: { level: 50, megaLevel: 0, megaLevelY: 3 }, target: { level: 50, megaLevel: 4, megaLevelY: 4 } },
    ];
  ...
  it("the worked two-Mewtwo example: summed energy + shared XL counted once", () => {
    ...
    const copies: PokemonCopy[] = [
      { id: "1", variant: "standard", current: { level: 40, megaLevel: 0, megaLevelY: 0 }, target: { level: 50, megaLevel: 4, megaLevelY: 4 } },
      { id: "2", variant: "standard", current: { level: 25, megaLevel: 3, megaLevelY: 3 }, target: { level: 50, megaLevel: 4, megaLevelY: 4 } },
    ];
  ...
  it("a single form keeps the whole leveling", () => {
    const x = mk("mega-mewtwo-x", { current: { candy: 0, xlCandy: 0, megaEnergy: 0, level: 40, megaLevel: 0 }, target: { level: 50, megaLevel: 4 } });
  ...
  it("variant raises the shared leveling (shadow 360)", () => {
    const copies: PokemonCopy[] = [
      { id: "a", variant: "shadow", current: { level: 40, megaLevel: 4, megaLevelY: 4 }, target: { level: 50, megaLevel: 4, megaLevelY: 4 } },
    ];
